...
def generate_ray_casting_grid_map(ox, oy, xy_resolution, breshen=True):
    ...
    if breshen:
        for (x, y) in zip(ox, oy):
            ...
            for laser_beam in laser_beams:
                if(laser_beam[0] + 1 < x_w and laser_beam[1] + 1 < y_w):
    ...
    # occupancy grid computed with with flood fill
        ...
        occupancy_map = init_flood_fill((center_x, center_y), (ox, oy), # substituir o center_x e center_y pela posição real do robô
                                        (x_w, y_w),
...
class MAPEAR(Node):
    ...
    def __init__(self):
        super().__init__('MAPEAR')
        self.get_logger().debug('Definido o nome do nó para "MAPEAR"')
    ...
    def listener_callback_laser(self, msg):
        ...
        self.incremento = msg.angle_increment
    ...
    def run(self):
        ...
        while (rclpy.ok):
            ...
            #[Não preciso criar uma lista de distancias, basta usar o self.ranges]
            #[Para pegar o angulo basta pegar o índice do angulo e multiplicar pelo self.incremento e somar ao self.minAng]

            #[posição dos obstáculos]
            # angulo = angulo minimo + incremento * indice
            ang =[self.minAng + self.incremento*i for i in range(len(self.ranges))]
            ox = np.sin(ang) * self.ranges
            oy = np.cos(ang) * self.ranges
